...
class sfForm implements ArrayAccess, Iterator, Countable
{
  ...
  public function renderHiddenFields($recursive = true)
  {
		...
		foreach ($this->getFormFieldSchema()->getWidget()->getFields() as $key => $object) {
			...
			if ($this->validatorSchema[$key] != null) {
  ...
  {
    /**
     * Trim ky va replace tu dac biet tu cach khi bind du lieu
     * @author NamDT5
     */
    if (!empty($taintedValues))
    {
      foreach ($taintedValues as $key => $val)
      {
        ...
        {
          if ($this->getValidator($key)->getOption('html_tag') === false)
          {
            // Loai bo the HTML neu co tinh insert vao truong khong cho phep nhap HTML
            $val = VtHelper::strip_html_tags($val);
          }
          if ($this->getValidator($key)->getOption('trim') === true)
          {
            // Trim du lieu neu truong validate trim
            $val = trim($val);
          }
          $taintedValues[$key] = VtHelper::replaceSpecialChar($val);
        } elseif (isset($val['text']) && $val['text']) {
          if ($this->getValidator($key)->getOption('html_tag') === false)
          {
            // Loai bo the HTML neu co tinh insert vao truong khong cho phep nhap HTML
            $val['text'] = VtHelper::strip_html_tags($val['text']);
          }
          if ($this->getValidator($key)->getOption('trim') === true)
          {
            // Trim du lieu neu truong validate trim
          ...
          $taintedValues[$key]['text'] = VtHelper::replaceSpecialChar($val['text']);
        }
      }
    }
    // END of NamDT5

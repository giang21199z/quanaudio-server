...
class sfValidatorDoctrineUnique extends sfValidatorSchema {
  ...
  protected function doClean($values) {
    ...
    //nghiald: bo xung chi check unique cho  truong hop value cua column la null hoac empty string
    $checkUnique = false;
    foreach ($this->getOption('column') as $column) {
      ...
      //nghiald: chi check unique khi value cua column khac null hoac empty tring
      if ($values[$colName] != null && $values[$colName] != '') {
//        $q->addWhere('a.' . $colName . ' = ?', $values[$column]);
        //tuanbm them 1 dong truy van de check phan biet tieng viet, dung ham is_numeric($test) ERROR voi so 45e1
        if (strtolower($values[$column]) === strtoupper($values[$column])) {
          $q->addWhere('a.' . $colName . ' = ?', $values[$column]); //truong hop nay chac chan chi toan number
        } else {
          $q->addWhere('a.' . $colName . ' = ?', $values[$column])//su dung 2 dong truy van de tim kiem nhanh hon
                  ->addWhere('( LOWER(a.' . $colName . ') = LOWER(?) COLLATE utf8_bin )', $values[$column]);
        }
        $checkUnique = true;
      }
    }
